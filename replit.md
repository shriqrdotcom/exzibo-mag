...
Better Auth's default id generator produces a 32-char alphanumeric string, not a UUID. `restaurants.owner_id` is a Postgres `uuid` column, so a real (non-DISABLE_AUTH) Better Auth user id would fail the insert with `invalid input syntax for type uuid`, surfacing as a generic "Restaurant creation failed" error. Fixed in `src/lib/auth.server.js` by setting `advanced.generateId: () => crypto.randomUUID()` so new Better Auth ids are valid UUIDs (no DB migration needed — Better Auth's own `user`/`session`/`account` id columns are `TEXT`). This does not retroactively fix ids already issued before the change, but no `user` rows existed in this database at the time of the fix.
