...
## Critical pitfall: DashboardApp must not reference `user` if not destructured
When removing `const { loading, user } = useAuth()` from DashboardApp, any JSX that references `user` will throw a ReferenceError (black page). Always replace with `<Navigate to="/auth" replace />` directly, or keep the destructure.
