...
import { Lock, Shield, ChevronDown, Check, Share2, Globe, ClipboardPaste, Link, Search, User, Phone, Mail, Layers, DollarSign, Clock, Copy, Calendar, X, Image, AlertCircle, CheckCircle2 } from 'lucide-react'
...
export default function Settings() {
  ...
  useEffect(() => {
    const key = 'exzibo_carousel_default'
    try {
  ...
  async function handleCarouselFiles(files) {
    ...
    if (currentCount >= MAX_GALLERY) { setGalleryError(`Maximum ${MAX_GALLERY} images allowed in the gallery.`); return }
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <main className="settings-main">
            ...
            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#666', marginBottom: '8px', paddingLeft: '4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              {galleryError && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '10px', padding: '8px 12px', marginBottom: '8px', color: '#EF4444', fontSize: '11px', fontWeight: 600 }}>
                  <AlertCircle size={12} /> {galleryError}
                </div>
              )}
              {gallerySuccess && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '10px', padding: '8px 12px', marginBottom: '8px', color: '#10B981', fontSize: '11px', fontWeight: 600 }}>
              ...
              {carouselImages.length === 0 ? (
                <div
                  ...
                  style={{
                    background: 'rgba(255,255,255,0.04)', borderRadius: '18px', height: '130px',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                    cursor: 'pointer', position: 'relative', gap: '6px',
                    border: '2px dashed rgba(255,255,255,0.15)',
                  }}
                >
                  <Image size={28} color="#555" strokeWidth={1.2} />
                ...
                <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '18px', height: '130px', position: 'relative', overflow: 'hidden' }}>
                  <img src={carouselImages[carouselIdx]} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '18px', display: 'block' }} />
                    ...
                    <>
                      ...
                      <div style={{ position: 'absolute', bottom: '8px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '4px' }}>
                        {carouselImages.map((_, i) => (
                          <div key={i} onClick={() => setCarouselIdx(i)} style={{ width: i === carouselIdx ? '14px' : '5px', height: '5px', borderRadius: '3px', cursor: 'pointer', background: i === carouselIdx ? '#fff' : 'rgba(255,255,255,0.5)', transition: 'all 0.2s' }} />
                        ))}
                      </div>
                    </>
                  )}
                  <button onClick={e => { e.stopPropagation(); carouselInputRef.current?.click() }} style={{ position: 'absolute', bottom: '10px', right: '10px', width: '28px', height: '28px', borderRadius: '8px', background: 'rgba(255,255,255,0.92)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '18px', color: '#555', boxShadow: '0 1px 4px rgba(0,0,0,0.15)', zIndex: 2 }}>+</button>
                  <button onClick={e => { e.stopPropagation(); removeCarouselImage(carouselIdx) }} style={{ position: 'absolute', top: '8px', right: '8px', width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', zIndex: 2 }}>
                    <X size={12} />
                  </button>
                </div>
              )}
            ...
            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '0.05em', textTransform: 'uppercase', color: '#fff', marginBottom: '10px', paddingLeft: '4px' }}>
                Description Text&nbsp;&nbsp;Carousel
              </div>
              <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px', padding: '14px 14px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setBadgeText(val)
                      localStorage.setItem('exzibo_carousel_badge_default', val)
                      window.dispatchEvent(new CustomEvent('exzibo-carousel-badge-changed', { detail: { restaurantId: 'default', badge: val } }))
                    }}
                    placeholder="ENTER BADGE TEXT…"
                    style={{ width: '100%', boxSizing: 'border-box', background: '#E8321A', border: 'none', borderRadius: '10px', padding: '7px 12px 7px 30px', fontSize: '11px', fontWeight: 700, color: '#fff', letterSpacing: '0.1em', textTransform: 'uppercase', outline: 'none', caretColor: '#fff', fontFamily: 'inherit' }}
                ...
                <textarea
                  ...
                  onChange={e => {
                    ...
                    localStorage.setItem('exzibo_carousel_desc_default', e.target.value)
                    window.dispatchEvent(new CustomEvent('exzibo-carousel-desc-changed', { detail: { restaurantId: 'default', text: e.target.value } }))
                  }}
                  placeholder="WRITE HERE"
                  rows={4}
                  style={{ width: '100%', boxSizing: 'border-box', background: '#fff', border: 'none', borderRadius: '12px', padding: '14px 16px', fontSize: '14px', fontWeight: 700, color: '#111', letterSpacing: '0.02em', resize: 'none', fontFamily: 'inherit', outline: 'none', lineHeight: 1.6, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}
                />
              </div>
            </div>
