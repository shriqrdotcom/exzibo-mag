...
import { Mail, Lock, Eye, EyeOff, Loader2, AlertCircle, FlaskConical } from 'lucide-react'
import { IS_PREVIEW } from '../lib/env'
import { previewLogin } from '../lib/previewAuth'

export default function Auth() {
  const navigate                        = useNavigate()
  const { signIn, signUp, signInWithGoogle, setPreviewUser } = useAuth()
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '28px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#666', fontWeight: 500 }}>
            {mode === 'login' ? 'Sign in to your Exzibo dashboard' : 'Start managing your restaurant today'}
          </div>
        </div>
        ...
        {error && (
          <div style={{
            ...
            background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', marginBottom: '20px',
          }}>
        ...
        {success && (
          <div style={{
            ...
            marginBottom: '20px', fontSize: '13px', color: '#22C55E', fontWeight: 500,
          }}>
            {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '14px' }}>
            <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#666', letterSpacing: '0.08em', marginBottom: '8px' }}>EMAIL ADDRESS</label>
            <div style={fieldWrap}
              onFocusCapture={e => e.currentTarget.style.borderColor = 'rgba(232,50,26,0.5)'}
              onBlurCapture={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'}
            >
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: '#666', letterSpacing: '0.08em', marginBottom: '8px' }}>PASSWORD</label>
            <div style={fieldWrap}
              onFocusCapture={e => e.currentTarget.style.borderColor = 'rgba(232,50,26,0.5)'}
              onBlurCapture={e => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'}
            >
              ...
              <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                ...
                placeholder={mode === 'login' ? 'Your password' : 'Min 6 characters'} minLength={6} style={inputStyle} />
              <button type="button" onClick={() => setShowPwd(p => !p)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#555', display: 'flex', padding: '2px' }}>
        ...
        <button type="button" disabled={loading} onClick={async () => {
          ...
          borderRadius: '12px', color: '#fff', fontSize: '14px', fontWeight: 600,
